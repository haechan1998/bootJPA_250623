...
public class FileSweeper {
    ...
    *  일치하는 파일(DB = file)은 남기고, (DB =! file) DB 에 없는 파일은 삭제
    *  직접 DB 에서 조회
    *  */
    ...
    public void fileSweeper(){
        ...
        // D:\\web_java_chc\\_myProject\\_java\\_fileUpload\\2025\\06\\26\\uuid_파일명"
        List<String> currentFiles = new ArrayList<>();
        for(File fvo : fileList){
            String filePath = fvo.getSaveDir() + java.io.File.separator + fvo.getUuid();
        ...
        log.info(">>>> DB FileList >>>> {}", currentFiles);

        // 실제 저장되어 있는 파일 목록과, DB 의 파일을 비교하여 DB 에 없는 파일은 삭제 진행
        // 오늘 날짜의 파일 경로 설정
        ...
        java.io.File direct = Paths.get(BASE_PATH + today).toFile();

        // 전체 객체를 배열로 나눠서 리턴
        java.io.File[] fileObject = direct.listFiles();
        log.info(">>>> Folder FileList >>>> {}", fileObject.toString());

        // 두 목록 비교 후 DB 에 없는 파일은 삭제
